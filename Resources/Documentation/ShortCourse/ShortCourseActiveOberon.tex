...
This is a brief introduction to the programming language Active Oberon and the operating system \AZ. This introduction does neither replace a language report nor is it an introduction to programming. It rather should provide a fast access to the \AZ system by ways of comments and examples. It has been prepared for the Operating System Course, Spring Semester 2009, at ETH Zurich as accompanying material to the practical exercises.
...
Basically, a module \verb+M+ is loaded when a command of the form \verb+M.CommandName+ is activated. Additionally, a module X is automatically loaded by the system when a module Y is loaded that imports X (cf.~ Paragraph \ref{section:ImportList}).

Once a module \verb+A+ has been loaded, any call of a valid command \verb+A.C+ will execute the loaded code and will not result in re-loading \verb+A+. The reason is clear: other modules may rely on information being provided globally in module \verb+A+ or \verb+A+ may contain state information that should not be removed after one call.
...
Type declarations bind an identifier to a type which can be used in variable declarations to name the type of the variables (see section \ref{section:VariableDeclarationSection}). Type declarations are used to provide synonyms for user-defined composite types or already existing types like for example all predeclared basic types (see sections \ref{section:CompositeTypes} and \ref{section:BasicTypes} respectively). The following example uses a type declaration to assign a new name to a composite record type.
...
Procedures contain the executable code of a module. They can be declared in a modules scope, within procedure scopes (`nested procedures') or in object scopes. Procedure parameters are special variables that refer to expressions that have been passed to the procedure from its caller. Parameters are declared in the head of the procedure declaration.
...
\subsubsection{SHORTINT, INTEGER, LONGINT and HUGEINT}\label{section:IntegerTypes}
...
\subsubsection{REAL and LONGREAL} \label{section:RealTypes}
...
b: POINTER TO ARRAY OF INTEGER; (* dynamic *)
...
Every module is considered to be an object type with a singleton instance, thus its procedures can also be protected. In this case the scope of protection is the whole module.
...
In this example, executing Example.CreateFile HelloWorld.txt Hello world ~ creates a new file (or overwrites an existing one) with the content "Hello World".
